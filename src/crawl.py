...
def fetch(url):
    """
    Fetch a URL. Prefer HTTPS. If TLS/hostname issues, try HTTP fallback.
    ...
    try:
        ...
        # Try scheme fallback https <-> http
        parsed = urlparse(url)
...
def crawl_candidate_pages(root_url):
    ...
    3) Parse root to discover anchors that look like contact/privacy/about
    """
    html_by_url = {}
    # 1. root
    try:
